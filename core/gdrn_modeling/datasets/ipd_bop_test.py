...
class IPD_BOP_TEST_Dataset(object):
    """ipd bop test splits."""
    ...
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg.get("dataset_root", osp.join(DATASETS_ROOT, "BOP_DATASETS/ipd/test"))
    ...
    def __call__(self):
        ...
        for scene_id, im_id in scene_im_ids:
            ...
            if scene_id not in cam_dicts:
                cam_dicts[scene_id] = mmcv.load(osp.join(scene_root, "scene_camera_cam1.json"))
...
SPLITS_IPD = dict(
    ipd_bop_test=dict(
        name="ipd_bop_test",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/ipd/test"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/ipd/models"),
        objs=ref.ipd.objects,
        ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/ipd/test_targets_bop19.json"),
...
# single obj splits for ipd bop test
for obj in ref.ipd.objects:
    for split in ["bop_test"]:
        ...
        ann_files = [
            osp.join(
                DATASETS_ROOT,
                "BOP_DATASETS/ipd/image_set/{}_{}.txt".format(obj, split),
            )
        ]
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_IPD:
        ...
        used_cfg = data_cfg
    DatasetCatalog.register(name, IPD_BOP_TEST_Dataset(used_cfg))
