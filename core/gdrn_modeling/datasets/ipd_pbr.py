...
class IPD_PRB_Dataset:
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg.get("dataset_root", osp.join(DATASETS_ROOT, "BOP_DATASETS/ipd/train_pbr"))
        ...
        self.height = 2400 #data_cfg["height"]  # 480
        self.width = 2400 #data_cfg["width"]  # 640
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.ipd.id2obj.items() if obj_name in self.objs]
        ...
        self.scenes = [f"{i:06d}" for i in [0,1,2,3,4,5]] #0,3,6,7,8,9,11,12,13,14,16,17,20,21,23,25,28,30,33,36,38,39,42,45 TODO Trinh,
        #1: 28,30,33,36
        #4: 30,33,36,38
        self.scenes = [f"{i:06d}" for i in range(14)]
    def __call__(self):
        ...
        for scene in tqdm(self.scenes):
            for cam in cam_list:
                ...
                if not osp.isdir(osp.join(scene_root, "rgb_" + cam)):
                    continue

                gt_dict = mmcv.load(osp.join(scene_root, "scene_gt_{}.json".format(cam)))
                gt_info_dict = mmcv.load(osp.join(scene_root, "scene_gt_info_{}.json".format(cam)))
                cam_dict = mmcv.load(osp.join(scene_root, "scene_camera_{}.json".format(cam)))

                for str_im_id in tqdm(gt_dict, postfix=f"{scene_id}"):
                    try:
                        int_im_id = int(str_im_id)
                        rgb_path = osp.join(scene_root, "rgb_{}/{:06d}.jpg").format(cam, int_im_id) #TODO Trinh
                        assert osp.exists(rgb_path), rgb_path

                        depth_path = osp.join(scene_root, "depth_{}/{:06d}.png".format(cam, int_im_id))
                        ...
                        for anno_i, anno in enumerate(gt_dict[str_im_id]):
                            ...
                            mask_file = osp.join(
                                scene_root,
                                "mask_{}/{:06d}_{:06d}.png".format(cam, int_im_id, anno_i),
                            )
                            mask_visib_file = osp.join(
                                scene_root,
                                "mask_visib_{}/{:06d}_{:06d}.png".format(cam,int_im_id, anno_i),
                            )
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(
                    self.models_root,
                    f"obj_{ref.ipd.obj2id[obj_name]:06d}.ply",
                ),
...
SPLITS_IPD_PBR = dict(
    ipd_train_pbr=dict(
        name="ipd_train_pbr",
        objs=ref.ipd.objects,  # selected objects
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/ipd/train_pbr"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/ipd/models"),
        xyz_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/ipd/train_pbr/xyz_crop"),
...
# single obj splits
for obj in ref.ipd.objects:
    for split in ["train_pbr"]:
        name = "ipd_{}_{}".format(obj, split)
        if name not in SPLITS_IPD_PBR:
            SPLITS_IPD_PBR[name] = dict(
                name=name,
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_IPD_PBR:
        used_cfg = SPLITS_IPD_PBR[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, IPD_PRB_Dataset(used_cfg))
...
def get_available_datasets():
    return list(SPLITS_IPD_PBR.keys())
